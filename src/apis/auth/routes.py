...
class Protected(Resource):
    ...
    def post(self):
        ...
        allowed_auds = Config.ALLOWED_AUDS if Config.ALLOWED_AUDS else []
...
# so user can't use access token to access to other api,
# also they can't use refresh token to create new access token
# docs:
# https://flask-jwt-extended.readthedocs.io/en/stable/blocklist_and_token_revoking/
@api.route("/logout")
class Logout(Resource):
    ...
    def post(self):
        ...
        # then we will use token_block_list to check revoked token
        # in function check_if_token_revoked
        token = get_jwt()
        jti = token["jti"]
        ttype = token["type"]
        ...
        ttype = "refresh"
        db.session.add(
            TokenBlocklist(jti=jti_refresh_token, type=ttype, created_at=now)
        )
